pipeline {
  ...
  steps { 
    ...
    sh "export JAVA_HOME=/usr/lib/jvm/java-8-openjdk-amd64; mvn org.owasp:dependency-check-maven:check -DfailOnError=false"
  }
}
    ...
    stage('Stage VII: Scan Image') {
      steps { 
        ...
        sh "trivy image --scanners vuln --offline-scan nehal-wandhare/democicd:latest > trivyresults.txt"
      }
    }
    ...
    stage('Stage VIII: Smoke Test') {
      steps { 
        ...
        sh "docker run -d --name smokerun -p 8080:8080 nehal-wandhare/democicd"
